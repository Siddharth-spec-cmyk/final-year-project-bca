...
from flask import Blueprint, Flask, render_template, request, send_file, send_from_directory, session, redirect, url_for
...
tools_app = Blueprint('tool_app', __name__)
...
@tools_app.route('/tool', methods=['GET', 'POST'])
def tools():
    ...
    if request.method == 'POST':
        ...
        # Save stock to user data
        users = load_users()
        username = session['user']
        if username in users:
            users[username].setdefault("stocks", []).append(stock)
            users[username]["stocks"] = list(set(users[username]["stocks"]))
            save_users(users)
...
@tools_app.route('/download/<filename>')
